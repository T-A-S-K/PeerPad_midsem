...
class CRDT {
  ...
  insertChar(char, pos) {
    ...
    if (char.value === "\n") {
      ...
      if (lineAfter.length === 0) {
        ...
        console.log("line after2-",pos.ch,char,pos.line)
      } else {
  ...
  findInsertIndexInLine(char, line) {
    ...
    if (char.compareTo(line[left]) === 0) {
      console.log("always left")
      return left;
    } else {
      console.log('ALWAYS RIGHT')
      return right;
    }
  }

  findPosBefore(pos) {
    console.log("find posss bef")
    let ch = pos.ch;
    let line = pos.line;
  ...
  generateChar(val, pos) {
    console.log("generate char-",val,pos)
    const posBefore = this.findPosBefore(pos);  
    const posAfter = this.findPosAfter(pos);
    console.log("possss=",posBefore,posAfter)
    const newPos = this.generatePosBetweenLetter(posBefore, posAfter);
     console.log("IMPPPP=",newPos)
  ...
  generatePosBetweenLetter(pos1, pos2, newPos=[], level=0) {
    console.log("betweeeennnSSS=",pos1,pos2)
    let id1 = pos1[0] || new Identifier(0, this.siteId);
    let id2 = pos2[0] || new Identifier(id1.digit+1, this.siteId);
    console.log("id1,id2--",id1,id2)
      let newDigit = this.generateIdBetweenLetter(id1.digit, id2.digit);
      newPos.push(new Identifier(newDigit, this.siteId));
      return newPos;
  }


generateIdBetweenLetter(indexStart, indexEnd) {
